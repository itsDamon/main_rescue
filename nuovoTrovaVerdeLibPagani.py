    ...
    if len(cnts) > 0:
        ...
        for c in cnts:
            ...
            if (w * h) < 500:  # da settare con quello reale
                continue
            if y < 30:  # pericolo sopra non vedo non posso valutare
                continue
            listaAree.append(nero(img, x, y, w, h))
            # se non arriva un verde vicino non inizio a curvare
            if y > 60:
                attivati = True
        if not attivati:  # non fare nulla
            return -1
...
def nero(img, x, y, w, h):
    ...
    (T, threshed) = cv2.threshold(gray, 80, 255, cv2.THRESH_BINARY_INV)  # converte in bianco e nero l'immagine
    ...
    sopra = copy[y - 20:y - 5, cx - 10:cx + 10]
    cv2.rectangle(img, (cx - 10, y - 20), (cx + 10, y - 5), (0, 0, 255))
